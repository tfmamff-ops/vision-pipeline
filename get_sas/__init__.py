...
ACCOUNT_URL = os.environ[
    "BLOB_ACCOUNT_URL"
]  # e.g., https://stvision109330379.blob.core.windows.net
...
def _make_upload_sas(
    container: str, blob_name: str, minutes: int, content_type: str | None
) -> str:
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if mode == "upload" and container != "input":
        return func.HttpResponse(
            "Uploads must target the 'input' container.", status_code=400
        )
    if mode == "read" and container not in ("output", "erp"):
        return func.HttpResponse(
            "Reads are allowed only from 'output' or 'erp' containers.", status_code=400
        )
    if not blob_name:
    ...
    return func.HttpResponse(
        json.dumps({"sasUrl": url}), status_code=200, mimetype="application/json"
    )
